...
function PitchLines() {
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <rect x="20" y="20" width="760" height="260" fill="none" stroke="white" strokeWidth="2" />
      <line x1="400" y1="20" x2="400" y2="280" stroke="white" strokeWidth="1.5" />
      <circle cx="400" cy="150" r="60" fill="none" stroke="white" strokeWidth="1.5" />
      <rect x="20" y="90" width="90" height="120" fill="none" stroke="white" strokeWidth="1.5" />
      <rect x="690" y="90" width="90" height="120" fill="none" stroke="white" strokeWidth="1.5" />
    </svg>
  );
}
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative h-64 bg-gradient-to-br from-emerald-950 via-emerald-800 to-teal-500 overflow-hidden flex items-end">
        ...
        <div className="relative z-[1] px-6 pb-6 w-full">
          <h1 className="text-4xl font-black tracking-tight text-white leading-none">{facility_name}</h1>
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                { icon: <span className="text-sm">⚽</span>, label: "Sport type", value: sports_type },
            ...
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {available_slots.map((slot) => {
                ...
                return (
                  ...
                  >
                    <Clock size={15} className={isSelected ? "text-white/80" : "text-emerald-500"} />
                    <div>
          ...
          <Card>
            <CardContent className="pt-5 space-y-4">
                  ...
                  {available_slots.map((slot) => {
                    ...
                    return (
                      <button
                        key={slot}
                        onClick={() => toggleSlot(slot)}
                        className={`flex items-center justify-between text-xs font-semibold px-3 py-2 rounded-lg border transition-all cursor-pointer w-full ${
                          isSelected
                      ...
                      >
                        ...
                        <span>{isSelected ? "✓ Selected" : "Tap"}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              >
                ...
                {selectedSlots.length ? "Confirm booking →" : "Select a slot to book"}
              </Button>
